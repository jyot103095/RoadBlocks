  ...
  checkBlocks() {
    for (let i = 0; i < this.blocks.length; i++) {
      ...
      if (block.positionY < 0) {
        ...
        break;
      } else if ((block.positionY + block.width) >= 335 && (block.positionY + block.width) <= 350) {
...
class GameView {
  ...
  drawGameOver() {
    ...
    this.ctx.fillRect(0, 0, 450, 700);
    this.ctx.fill();
    this.ctx.font = "26px Do Hyeon";
    this.ctx.strokeStyle = "cyan";
    this.ctx.textAlign = "center";
    this.ctx.strokeText(`Oh man! You only scored ${this.game.score} points.`, 225, 300);
