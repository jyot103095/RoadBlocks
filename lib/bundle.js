...
class Game {
  ...
  checkBlocks() {
    for (let i = 0; i < this.blocks.length; i++) {
      ...
      if (block.positionY < 0) {
        continue;
      } else if (block.positionY > 600) {
        this.blocks.splice(i, 1);
        break;
      } else if ((block.positionY + block.width) >= 390 && (block.positionY + block.width) <= 400) {
  ...
  draw(ctx) {
    ...
    if (this.getHighestBlock() && this.getHighestBlock().positionY > 450) {
      this.generateBlocks();
    }
