...
class Game {
  ...
  draw(ctx) {
    ...
    if (this.getHighestBlock()) {
      if (this.score > 150) {
        distanceBetweenBlocks = 300;
      }

      if (this.getHighestBlock().positionY >= distanceBetweenBlocks) {
...
const generateBlocks = (snakeLength) => {
  ...
  let lowBlockValue = Math.floor((Math.random() * snakeLength) + 1);
  let lowValueBlock = new _block__WEBPACK_IMPORTED_MODULE_1__["default"](positionsArr[0], lowBlockValue);
