...
class Break {
	...
	generateParticles() {
		...
		for (let i = 0; i < 10; i++) {
			...
			particles.push(new _particle__WEBPACK_IMPORTED_MODULE_0__["default"](this.posX, this.posY, randX, randY));
		}
      ...
      if (block.positionY < 0) {
        ...
        if (this.calculateDistance(block) <= (block.width + this.snake.radius)) {
          if (this.snake.tail[0].positionX - block.positionX >= 0 && (this.snake.tail[0].positionX - block.positionX) <= block.width) {
            ...
            this.breaks.push(new _break__WEBPACK_IMPORTED_MODULE_3__["default"](this.snake.positionX, block.positionY + block.width));
            break;
          }
        }
      }
...
class GameView {
  ...
  drawGameOver() {
    ...
    this.ctx.strokeStyle = "cyan";
    this.ctx.textAlign = "center";
    this.ctx.strokeText(`You scored ${this.game.score} points!`, 225, 250);
