...
class Game {
  constructor() {
    ...
    this.velocity = 2;
    this.points = [];
    this.blocks = [ new _block__WEBPACK_IMPORTED_MODULE_2__["default"](0, this.velocity), new _block__WEBPACK_IMPORTED_MODULE_2__["default"](270, this.velocity), new _block__WEBPACK_IMPORTED_MODULE_2__["default"](360, this.velocity) ];
  }
...
class Snake {
  ...
  move(blocks) {
    ...
    for (let i = 1; i < this.length; i++) {
      ...
      let newPrevPos = tailEl.positionX;
      tailEl.updateHorizontalPosition(prevPos, blocks);
      prevPos = newPrevPos;
      let distance = Math.sqrt((tailEl.positionX - prevEl.positionX)**2);
      let velocityX = (tailEl.positionX - prevEl.positionX) / 2;

      // if (distance < velocityX) {
      //   tailEl.updateHorizontalPosition(prevEl.positionX, blocks);
      // }
      // else if ((tailEl.positionX + velocityX) >= 10 && (tailEl.positionX + velocityX) <= 440) {
